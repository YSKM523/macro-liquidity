...
describe('ingest repository contracts', () => {
  ...
  it('promotes observations and switches ACTIVE with exactly one D1 batch', async () => {
    ...
    expect(statements[3].sql).toMatch(/INSERT INTO market_prices_daily[\s\S]*FRED:/i);
    expect(statements[4].sql).toMatch(/INSERT INTO cash_rates_daily[\s\S]*FRED:SOFR/i);
    expect(statements[5].sql).toMatch(/SUPERSEDED[\s\S]*ACTIVE/i);
    expect(statements[5].sql).toMatch(/EXISTS[\s\S]*state\s*=\s*'RUNNING'/i);
    expect(statements[6].sql).toMatch(/state = 'ACTIVE'/i);
    expect(statements[7].sql).toMatch(/SELECT[\s\S]*ingest_lock[\s\S]*'now'/i);
  });
