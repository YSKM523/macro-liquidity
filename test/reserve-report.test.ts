...
describe('reserve challenger report disclosure', () => {
  it('labels source correction, current-vintage evidence, frozen gate, freshness, and no production impact', () => {
    ...
    for (const phrase of ['A-001', 'A-002', 'TGCRRATE', 'NY Fed', '2021-07-29', 'small-value exercises', 'RESEARCH_CURRENT_VINTAGE', 'current-vintage retrospective pseudo-OOS', 'GDP observation-date alignment is not release-aware', 'not ALFRED/PIT', 'DATA_INCOMPLETE', 'replacementEligible: `false`', 'Champion: unchanged', 'DROP_RESEARCH']) expect(markdown).toContain(phrase);
  });
});
