...
> 用美联储的「钱多钱少」判断美股顺不顺风。**8 个宏观计分因子** → **0–100 顺风指数** 和 **偏多 / 中性 / 偏空** 红绿灯,外加 **1 个独立实时风控覆盖层**和**「操作建议 · 仓位旋钮」**。
...
- **FRED = 宏观历史与模型逻辑的唯一真相源**；实时行情层按 Yahoo → Stooq（可用时）→ FRED 官方序列降级，只影响顶部读数和 live-stress、**不入库且不改变宏观分**。FRED 映射为 SPX=`SP500`、VIX=`VIXCLS`、DXY=`DTWEXBGS`、10Y=`DGS10`。每个结果分别携带实际 instrument/provider 的行情时间和抓取时间；无合法 provider 时间戳时不会拿抓取时间代替。所有 FRED/ALFRED 与实时 provider HTTP 共用有界重试：仅 transport、429、5xx 最多尝试 3 次并使用带 jitter 的封顶指数退避；其它 4xx、解析或验证失败立即失败/降级。
...
### 2）8 个宏观计分因子(权重由 10 年回测的因子 IC 校准,非按 IC 硬拟合)
...
`vol` 是基于正式 FRED `VIXCLS` 的旧版零权重宏观诊断（`LEGACY_ZERO_WEIGHT_DIAGNOSTIC`），为兼容历史快照保留，但不参与 0–100 分，也不是独立实时风控覆盖层。live-stress 覆盖层另行使用 VIX / SPX / 10Y / DXY 的实时多源输入。
...
市场大多数时候在涨,所以**方向命中率是个误导性指标** —— 想把它拉到 76% 的唯一办法是让模型永远偏多,那就废了。**本模型的价值在排序 / 风控**:它更看空时,未来收益更低、回撤更集中。历史 **Sharpe 1.10 属于 `LEGACY_WEEKLY` 诊断，不是正式 event-time 指标**。**正确的记分牌是 IC / 风险调整表现 / 躲回撤,不是命中率。**

---

## 验证(诚实的样本外检验)

- **`/api/backtest`** —— 正式绩效使用 event-time：只有 `tradable_at` 严格早于某交易日 `17:00:00Z` 的保守最早美股收盘界线，才使用该日实际 SPX 日线；否则等待下一条实际日线。返回的 `23:59:59Z` 仅为日频记账标记，不是交易所实际收盘时间戳。市场/现金 revision append-only，`?as_of=<canonical ISO>`（默认 D1 当前时间）让 signal `recorded_at` 和 daily `activated_at` 共用一个 strict-visibility cutoff；只选择时间**严格早于** cutoff 的最新 revision，因此同毫秒 commit 会保守延后一请求。正式结果标记 `APPEND_ONLY_AS_OF` / `responseReproducible=true`；任何 synthetic、legacy/no-PIT 或缺失 provenance/policy 输入均返回 typed `DATA_INCOMPLETE`。正式仓位采用共享 `DASHBOARD_EXPOSURE_TIERS_V1` 的 100/90/75/50/25% long/cash 档位；历史 stress 只读冻结周快照 `vix_eod`（`PIT_SNAPSHOT_VIX_PROXY`）。策略与 100% SPX、平均 Beta 静态组合、prior-only 20 日 10% 波动目标、prior-close 200DMA 基准共用同一绩效窗口、SOFR 与成本；波动/均线估计可使用 cutoff 前且窗口前的可见 warm-up。报告收益、相对 Beta 静态组合的累计择时收益差、Beta、波动、Sharpe、Sortino、最大回撤及持续期。旧周频 long/flat 仅为 `LEGACY_WEEKLY` 诊断。
...
- **PR-12 动态准备金充裕度 Challenger** —— `PR12_RESEARCH_V2_SRF_BOUNDARY`以 prior-only expanding percentiles 合成相对准备金、13 周变化、SOFR−IORB 与辅助融资压力，独立 freshness 失败即 `DATA_INCOMPLETE`。这些诊断只是 current-vintage retrospective pseudo-OOS；GDP observation-date alignment is not release-aware。A-002 把 NY Fed SRF 边界固定为 `2021-07-29`，v1 因混入历史临时 Repo 而 `INVALIDATED_BY_REVIEW`；小额 exercises 作为限制保留。v2 overlapping/non-overlapping IC 为 0.2363/−0.0071（n=194/15），只有 3 个正 fold 且 top-quintile 10% tail 更差，冻结结论 `DROP_RESEARCH`、`RESEARCH_CURRENT_VINTAGE`、`replacementEligible=false`，Champion 不变。报告见 [`docs/research/RESERVE_ADEQUACY_OOS_REPORT.md`](docs/research/RESERVE_ADEQUACY_OOS_REPORT.md)。
...
- 测试:**Vitest**(518 测试，覆盖模型逻辑、原子摄取、PIT/vintage、event-time 日频回测、冻结 endpoint audit index、provider fallback、锁与 API)
- 部署:`wrangler`
...
src/
  ...
  backtest.ts     旧周频 IC / 命中率 / LEGACY_WEEKLY 诊断
  event-backtest.ts  正式 event-time 调度 / 日频 NAV / SOFR / 成本
