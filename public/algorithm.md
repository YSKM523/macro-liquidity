...
- `cron`(每 3 小时)→ FRED 增量 → 重算近 14 天 `nowcast_snapshot_daily`，并明确标记为 `PROVISIONAL`；全量回填按 WALCL **周线**生成 `model_snapshot_weekly`。
- 存储:D1 `observations`(FRED 长表)/ `model_snapshot_weekly`(正式周频信号)/ `nowcast_snapshot_daily`(周中预估)。回测、walk-forward 与 robustness 只读取正式周频表。
