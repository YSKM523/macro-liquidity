...
| 权重方案 | OOS IC | 命中率 |
|---|--:|--:|
| WF 自动拟合(train 内 IC 定权) | **0.052** | 48% |
| 等权(零调参,不偷看数据) | **0.206** | 66% |
| 当前手调权重(有 in-sample 污染,别全信) | 0.376 | 64% |

各 fold 的 WF IC 在 **−0.35~+0.44** 剧烈摆动(短样本噪音,2020/2023 反向)。**三条硬结论:**
1. **别让数据自动选权重** —— 自动按 IC 调权在 10 年样本上过拟合,OOS 塌到 ~0。
2. **edge 在"因子选择",不在"精确权重"** —— 零调参的等权就有 OOS IC ~0.21(≈全样本 0.23);**选对因子 > 精调权重**。
3. **别把权重当精密科学**:等权与手调在样本外差不多。
> 注:上表的高命中率(64–66%)只在 walk-forward 的 test 子段成立;**放回全历史是 ~53%**(刚过抛硬币)。代表性命中率以 §6 的全样本 ~52% 为准。
