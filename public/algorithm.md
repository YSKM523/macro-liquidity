...
正式绩效块为 `event_time`：只接受 `decision_status=OK`、`pit_status=PIT` 且具有 `decision_at/tradable_at` 的周频正式信号，在 `tradable_at` **严格之后**第一个实际 SPX 日线的 `23:59:59Z` 收盘执行；同一收盘只采用最新 `decision_at`。净值逐个实际交易日按前一收盘敞口结算，空闲现金使用最新已知 SOFR、ACT/360 简单计息。每单位换手收 1bp 手续费 + 2bps 基础滑点；VIX≥28、VIX 陈旧或缺失时再加 3bps。超过 100% 敞口的通用引擎按 SOFR + 100bps 融资，但本 PR 的兼容目标仍只是 `score>55 ? 100% : 0%`。SPX `adjusted_close` 明确是 FRED SP500 指数收盘，不代表含股息总回报。

SOFR 缺失/超过 4 个日历日、没有可执行信号或不足两个执行后交易 session 时，`event_time` 返回 `DATA_INCOMPLETE`，`totalReturn=null`，不展示部分或伪零绩效。旧 `strategy_long_flat` 继续服务历史比较，但显式标为 `LEGACY_WEEKLY`，不再是正式绩效路径。
...
- `cron`(每 3 小时)→ 用 D1 当前时间获取数据库租约 → 建立序列 attempt → 以 ALFRED `output_type=3` 和 inclusive `realtime_start` 抓取 vintage → 双 staging → 完整性校验 → 单个 D1 事务更新兼容 `observations`、append-only `observations_pit`、SPX/VIX 日收盘与 SOFR 现金表并切换唯一 ACTIVE run → 按逐日 event-time cutoff 重算近 14 天 nowcast。checksum 冲突、租约转移或任一语句失败都会让整批回滚。
