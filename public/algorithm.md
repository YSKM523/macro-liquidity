...
行情与近 5 日历史统一走 typed provider contract：Yahoo 为主源，SPX/VIX/DXY 以 Stooq 为备用，10Y 以 FRED DGS10 为备用。结果分别记录 provider 的 `sourceTimestamp` 与调用方 `fetchedAt`，周末/收盘后不会把旧行情标成刚更新。数据质量状态为 `OK / STALE / DIVERGENT / FAILED`；主备源仅在共同日期窗口比较标准化变化，超出 `MARKET_DATA_QUALITY` 命名容差时返回 `SOURCE_DIVERGENCE`。任何必需 stress 输入非 `OK` 都 fail closed 为 `UNKNOWN`，有效 fallback 则可继续使用。以上容差只用于数据质量，不属于 Champion 评分或 stress 触发阈值。
...
- `cron`(每 3 小时)→ 用 D1 当前时间获取数据库租约 → 在生产序列读取前建立每个 FRED 序列 attempt → 写入本次 `run_id` 的 staging → 完整性校验 → 单个 D1 事务更新 `observations` 并切换唯一 ACTIVE run → 重算近 14 天 `nowcast_snapshot_daily`。获取、续租与所有生产 mutation 都在 SQL 内用数据库当前时间验证 owner 和未过期状态；激活事务另有 batch 内终止 fence，失去 ownership 会让整批回滚。手动 refresh 与 cron 争用时显式返回冲突。
- 存储:D1 `ingest_runs` / `ingest_series_attempts` / `staging_observations` 保存运行审计，ACTIVE run 另有 `snapshot_state=PENDING|SUCCEEDED|FAILED`、完成时间、错误与写入计数；`observations` 继续作为生产最新视图。成功 metadata 与 `snapshot_state=SUCCEEDED` 在同一个 fenced D1 batch 发布，因此 health 不会看到成功 metadata 配合 `PENDING` outcome；ACTIVE snapshot 非 `SUCCEEDED`（包括 `PENDING`）均返回 503 和显式原因。`model_snapshot_weekly` 是正式周频信号，`nowcast_snapshot_daily` 是周中预估；回测、walk-forward 与 robustness 只读取正式周频表，staging 不会进入分析。
