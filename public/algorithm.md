...
完全前瞻 holdout 于 `2026-07-22T19:37:28Z`、commit `75c93d526bf6073440335d3c90a7d5c0b90ea58b` 登记，起点固定为 `2026-07-23`。digest 绑定固定 200/52 folds、purge/embargo、entry/exit、Spearman IC、八个评分因子、Champion model/config、仓位方法和风险阈值。登记时没有诚实可冻结的训练权重或 q10，因此前瞻 tail 永久为 `UNAVAILABLE_AT_REGISTRATION/null`，不得事后补冻；方向/verdict/risk/IC 直接评估登记的固定 Champion。至少 5 个 post-registration、按 execution date 归属的 13 周 label 成熟前状态为 `PENDING_MATURITY`，历史尾部不算 unseen。post-registration 信号必须是与登记 model/config 完全一致的 governed cohort；migration 0010 的既有 PIT 历史只能诚实提供 `PARTIAL_LEGACY` retrospective 指标，legacy calibration 的 tail 为 `PARTIAL_LEGACY_CALIBRATION/null`。非 PIT、日价 provenance 不完整、畸形或混合 governed cohort 均 fail closed。
