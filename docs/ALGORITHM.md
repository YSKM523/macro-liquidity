...
PR-15 在旧三臂诊断旁新增 `PURGED_VALIDATION_V1`，不改动下表任何旧值。每条 13 周 label 显式记录 `signalDate/outcomeDate`；训练集先剔除 `outcomeDate >= testFrom` 的 label，再按日期执行 91 日 embargo，不以行数近似。每 fold 同时报告重叠样本和贪心区间非重叠样本。Champion 指标始终使用持久化 score、正式 verdict 和由既有 `DASHBOARD_EXPOSURE_TIERS_V1` 映射得到的 target exposure；训练内 IC 权重仅作为单列 diagnostic，不替换 Champion。

五类结果为：score 相对 50 的方向命中（等于 50 或零收益 abstain）、持久化正式 verdict 命中（`NEUTRAL` abstain）、`targetExposure <= 0.50` 风险调用的 precision/downside recall、score/return Spearman IC、以及各 fold 仅用已成熟训练 outcome 校准的 q10 尾部 recall/precision。率至少需要 5 个 eligible，IC 至少 3 个且 score/return 都须有方差，tail 至少需要 20 个训练 outcome 与 3 个测试 tail event；否则返回带 status 的 null，绝不伪造 0 或 `NaN`。

完全前瞻 holdout 的协议 digest 和起点 `2026-07-23` 已在代码中冻结；训练 outcome cutoff 固定为 `2026-04-23`，后续数据不得移动起点、权重或 q10。至少 5 个 post-registration 13 周 label 成熟前状态为 `PENDING_MATURITY`，历史尾部不算 unseen。migration 0010 的既有 PIT 历史可诚实提供 `PARTIAL_LEGACY` retrospective 指标，但 legacy calibration 的 tail 为 `PARTIAL_LEGACY_CALIBRATION/null`；非 PIT、畸形 provenance 或 post-holdout legacy 信号 fail closed。
