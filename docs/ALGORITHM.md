...
PR-15 在旧三臂诊断旁新增 `PURGED_VALIDATION_V1`，不改动下表任何旧值。正式 label 不读取周快照的同日 SPX：每条周频信号按 `decisionAt/tradableAt` 找第一条合资格 PIT 日收盘作为 entry，再找 entry 后第 91 个日历日当日或之后第一条实际 PIT 日收盘作为 exit，并显式记录 `modelDate/decisionAt/tradableAt/entryDate/exitDate`。训练集先剔除 `outcomeDate >= testFrom` 的 label，再对 outcome 执行 91 日 embargo，不以行数近似。每 fold 同时报告重叠样本和按最早 exit 贪心选择的半开收益区间 `(entry, exit]` 非重叠样本。Champion 指标始终使用持久化 score、正式 verdict 和既有 `DASHBOARD_EXPOSURE_TIERS_V1` target exposure；训练内 IC 权重仅作为单列 diagnostic，不替换 Champion。

五类结果为：score 相对 50 的方向命中（等于 50 或零收益 abstain）、持久化正式 verdict 命中（`NEUTRAL` abstain）、`targetExposure <= 0.50` 风险调用的 precision/downside recall、score/return Spearman IC、以及各 fold 仅用已成熟训练 outcome 校准的 q10 尾部 recall/precision。风险 precision 把零收益 risk call 计为 false positive；downside recall 的分母只含负收益。率至少需要 5 个 eligible，IC 至少 3 个且 score/return 都须有方差；tail 需要至少 20 个训练 outcome，recall 至少 3 个测试 tail event，precision 至少 3 个 risk call。否则返回带 status 的 null，绝不伪造 0 或 `NaN`。

最初 `2026-07-22T19:37:28Z` / `75c93d5` 的登记因仍使用周频 pre-decision SPX、signal-date embargo 和事后 tail calibration，已明确记为 `INVALIDATED_BY_REVIEW`。修订协议于 `2026-07-22T20:17:47Z` 锚定 implementation commit `31d26408ec6a3e05ef6da9ce7a9277320dcbf8f9`，起点仍在未来、按 execution date 固定为 `2026-07-23`；active digest literal 为 `80092bd0142ae4faf8e62f00ec7ccb3e8b6c0d94bd1a9944110833ec372f8b28`。它绑定固定 200/52 folds、purge/embargo、entry/exit、Spearman IC、八个 literal 评分因子、literal Champion model/config、仓位方法和风险阈值；代码若与 literal 不符会要求显式新 amendment，而不会静默改写注册。修订时没有诚实可冻结的训练权重或 q10，因此前瞻 tail 永久为 `UNAVAILABLE_AT_REGISTRATION/null`，不得事后补冻；方向/verdict/risk/IC 直接评估登记的固定 Champion。至少 5 个 post-registration、按 execution date 归属的 13 周 label 成熟前状态为 `PENDING_MATURITY`，历史尾部不算 unseen。所有输入信号（包括 superseded）须通过身份/时钟/政策校验；缺价格、无执行或 unexecuted coverage fail closed，valid superseded 仅计数而不进入收益 label。migration 0010 的既有 PIT 历史只能诚实提供 `PARTIAL_LEGACY` retrospective 指标，legacy calibration 的 tail 为 `PARTIAL_LEGACY_CALIBRATION/null`。
