...
- `cron`(每 3 小时)→ 获取数据库租约 → 每个 FRED 序列写入本次 `run_id` 的 staging → 完整性校验 → 单个 D1 事务更新 `observations` 并切换唯一 ACTIVE run → 重算近 14 天 `nowcast_snapshot_daily`。手动 refresh 与 cron 争用时显式返回冲突；失败 run 不写快照。
- 存储:D1 `ingest_runs` / `ingest_series_attempts` / `staging_observations` 保存运行审计，`observations` 继续作为生产最新视图；`model_snapshot_weekly` 是正式周频信号，`nowcast_snapshot_daily` 是周中预估。回测、walk-forward 与 robustness 只读取正式周频表，staging 不会进入分析。
